...
public class SurpriseMe extends JPanel implements ActionListener, Runnable, java.io.Serializable {
	...
	public SurpriseMe( Semester s, ScheduleGUI d, String gerChoice){
		...
		if(CourseList.getCoursesIn(s).size()==0){
			...
			JOptionPane.showMessageDialog(frame, "Classes are not avaliable in the "+ s.semesterDate.getSeason(s.semesterDate.sNumber)+ " "+ s.semesterDate.year + " semester", "No classes",JOptionPane.INFORMATION_MESSAGE,  icon  );
		...
		if(gerChoice == null){
			...
			for(Requirement r: d.sch.getGER().reqList){
		...
		if(gerChoice != null){
			...
			if(s.semesterDate.sNumber != (SemesterDate.MAYX)){
				...
				getReady = CourseList.onlyThoseSatisfying(almostReady, d.sch.getGER().getRequirement(gerChoice));
				if(getReady.size()>0){
			...
			ScheduleCourse cc = new ScheduleCourse(c, d.sch);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getActionCommand().equals(MenuOptions.tryAgain)){
			...
			SurpriseMe s = new SurpriseMe(this.s, this.d, gerChoice);
