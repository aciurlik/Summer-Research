...
/**
 * 
 * Blurb written 7/26/2017 Last updated 7/26/2017
 * 
 * This is the highest level of code. It holds all of the scheduleGUIs that the
 * program has made. It is also holds the main method run by the program. If any
 * action is not specific to a schedule/schedule, or an entity that is smaller
...
public class Driver {
	...
	static ArrayList<ScheduleGUI> listOfScheduleGUIs; // This allows multiple
														// schedules to be
														// opened
	// without them interacting with each other.
	static StartUpMenu startUP = null;// This stores the startup object, because
										// inital loading is time
	// consuming.
	static PriorData data;

	/**
	 * When one creates a new Schedule through initial loading of the program,
	 * or when they open a program this method is called. It adds that program
	 * to an ArrayList list of scheduleGUI's and displays it in a separate
	 * window. When a schedule is created from scratch it replaces the current
	 * schedule, and thus this method is not called.
	 * 
	 * @param s
	 *            Schedule that one wants to open.
	 */
	public static void addScheduleGUI(Schedule s) {

		ScheduleGUI schGUI = new ScheduleGUI(s);
		// The window listeners keep the closure of one window from causing the
		// others to close.
	...
	/**
	 * This displays a dialog box that reminds the user that they have the
	 * option to save
	 * 
	...
	public static int saveScheduleReminder() {
		...
		int n = JOptionPane.showOptionDialog(null, "Would you like to save your schedule", "Save Schedule",
				JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null, Options, Options[1]);
		return n;
	}

	/**
	 * This closes the schedule, and if it is the last one proceeds to close the
	 * whole program.
	 * 
	 * @param schGUI
	 *            ScheduleGUI that one wants to close.
	 */
	...
	/**
	 * When one opens a schedule it defers to FileHandler to pick the schedule,
	 * once the user selects the schedule, as long as it is not null the schGUI
	 * is created and added to the list this class holds. If master is around
	 * the majors that were saved will be updated to their most updated status.
	 * Otherwise if a major file was updated after a schedule was saved, when
	 * this saved schedule was opened it would not reflect that change.
	 */
	...
	/**
	 * This displays a list of string to the user, and lets the user choose
	 * among them. This returns the index of the chosen string, -1 if none
	 * chosen. Strings should be unique, if not the first will be chosen.
	 * 
	...
	public static int GUIChooseAmong(ArrayList<String> displaysList, String message, String title) {
		...
		String chosenString = (String) JOptionPane.showInputDialog(null, message, title, JOptionPane.PLAIN_MESSAGE,
				null, displays, "Cats");
		int chosenIndex = 0;
	...
	/**
	 * This allows the user to pick a start semester at the start of the
	 * program. (The first blank schedule creates this option for the user). The
	 * program then stores this answers and if another blank schedule is loaded
	 * its start date will be but what the user selected. If the user shuts down
	 * the program and then reopens it they must pick the start date again for a
	 * blank schedule.
	 * 
	 * @param semesters
	 * @return SemesterDate that will start the schedule. Null if user does not
	...
	public static SemesterDate tryPickStartDate() {
		...
		// Sets the stored default semester to the user choice. If it is not
		// null.
		if (result != null) {
	 ...
	 */
	private static String preScheduleLoading() {
		String errorMessage = " ";
		try {

			if (FileHandler.propertyGet(MenuOptions.startUp).equals("true")) {
				errorMessage = errorMessage + "APP is having trouble loading. Please exit out of the program; ";
				errorMessage = errorMessage + System.lineSeparator() + "then make sure the file entitled 'Resources' " + System.lineSeparator() + "is inside a single folder with the .jar file you clicked on.";
	...
	/**
	 * This disclaimer is shown at the start of the program. It is also
	...
	/**
	 * This the following is a work in progress it is not linked to the schGUI,
	 * the only way one can access this is by turning on master.
	 */
	///////////////////////////////////////////////////////////////////////////
	////////////////////// UNUSED CODE/ | ///////////////////////////////////
	////////////////////////////////// v /////////////////////////////////////
	...
	/**
	 * This is what initiates the whole program. It displays the disclaimer/load
	 * screen And then creates and displays the first scheduleGUI. If the
	 * setting say to display the startUp guide it will also do that.
	 *
	 * 
	 */
	public static void main(String[] args) {
		...
		/**
		 * This is an attempt to set the apple controlled menubar to something
		 * other than the class name. It does not work because we are using Java
		 * 8. On the list of things to look into.
		 */
		...
		confirm.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (e.getActionCommand().equals(MenuOptions.confirm)) {
					...
					// hopefully enticing the user to play with the program
					// behind it
		...
		try {
			// This gives the user an update on the status of loading the
			// backround files of the program
			editorPane.setText("<html><body>" + Driver.getDisclaimer()
					+ "<br />  <br /> Establishing Settings <br />-           " + "</p></body></html>");
			PerrorMessage = preScheduleLoading();
			listOfScheduleGUIs = new ArrayList<ScheduleGUI>();
			editorPane.setText("<html><body>" + Driver.getDisclaimer()
					+ "<br />  <br /> Courses Loading <br />---             " + "</p></body></html>");
			CourseList.loadAllCourses();
			editorPane.setText("<html><body>" + Driver.getDisclaimer()
					+ "<br />  <br /> Majors Loading <br />-----        " + "</p></body></html>");
			FileHandler.getMajorsList();
			editorPane.setText("<html><body>" + Driver.getDisclaimer()
					+ "<br />  <br /> Finished Loading <br />----------- 100%" + "</p></body></html>");
		} catch (Exception e) {
			JFrame error = new JFrame("APP Loading Error");
			e.printStackTrace();

			JLabel errorMessage = new JLabel(PerrorMessage);
